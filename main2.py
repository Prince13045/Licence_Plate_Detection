...
# Matches 2 letters, 2 numbers, an optional space, and 3 letters
plate_pattern = re.compile(r'^[A-Z]{2}[0-9]{2}\s?[A-Z]{3}$')
...
def correct_plate_format(ocr_text):
    ...
    # Standard UK current format is exactly 7 characters
    if len(ocr_text) != 7:
        return ""
    ...
    for i, ch in enumerate(ocr_text):
        ...
        if is_letter_position:
            if ch.isdigit() and ch in mapping_num_to_text:
                ...
                return ""
        else: # Must be a number (indices 2 and 3)
...
def get_stable_plate(track_id, new_text):
    if new_text:
        ...
        plate_final[track_id] = most_common  # Fixed: Save it to final mapping
    return plate_final.get(track_id, "")

# Video I/O setup
# --- VIDEO FOR INFERENCING SETUP ---
input_video = r"C:\Liscenceplatedetection\data\inputvideo1.mp4"
output_video = r"C:\Liscenceplatedetection\data\output2.avi"  
...
while cap.isOpened():
    ...
    # Force resize the frame right before writing to make 100% sure it matches VideoWriter configurations
    final_frame = cv2.resize(frame, (frame_width, frame_height))
    out.write(final_frame)
    
    print(".", end="", flush=True)

# Safe Release (No GUI dependency strings)
cap.release()
out.release()
